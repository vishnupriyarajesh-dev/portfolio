...
const TiltCard = ({ project }: { project: typeof projects[0] }) => {
  ...
  return (
    <div
      ...
      className="bg-white border border-gray-200 rounded-2xl p-6 flex flex-col justify-between shadow-sm hover:shadow-xl hover:border-amber-400 transition-all duration-300"
    >
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech.map(t => (
            <span key={t} className="bg-amber-50 text-amber-700 text-xs px-3 py-1 rounded-full border border-amber-200">{t}</span>
          ))}
        </div>
      </div>
      <div className="flex gap-4">
        <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-500 hover:text-amber-600 transition-colors font-medium">GitHub →</a>
        {project.live && <a href={project.live} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-500 hover:text-amber-600 transition-colors font-medium">Live →</a>}
      </div>
    </div>
  )
}
  ...
  return (
    <section id="projects" className="py-24 px-6 bg-white">
      <div ref={ref} className="fade-up max-w-6xl mx-auto">
        <p className="text-amber-600 uppercase tracking-widest text-sm mb-3 text-center font-semibold">My Work</p>
